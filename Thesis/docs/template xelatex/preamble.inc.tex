...
% Внутренние отступы ячеек: горизонтальные 6pt с каждой стороны, вертикальные +20%
...
% \small (используется в таблицах) — 12pt + одинарный межстрочный интервал
% Нельзя использовать \setstretch — она вызывает \@currsize, что даёт бесконечную рекурсию.
% Напрямую выставляем \baselinestretch и перезапускаем \selectfont.
...
\newcommand{\appendixletter}{\Asbuk{appendixnum}}
...
\setminted{
    style=bw,               % без цветной подсветки синтаксиса
    ...
    encoding=utf8,          % явно фиксируем UTF-8 для pygments/minted на Windows
    fontfamily=tt,          % использовать моноширинное семейство
    fontsize=\small,
    breakanywhere=true,
...
% minted v3 не имеет ключа для стиля номеров — переопределяем через fancyvrb
...
% Убираем межстрочный пропуск после xltabular/longtable
\setlength{\LTpost}{0pt}
% \AtBeginDocument{\setlength{\LTpre}{\baselineskip}}
