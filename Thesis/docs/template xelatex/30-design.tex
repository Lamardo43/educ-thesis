...
Для формализованного описания архитектуры применяется нотация C4~Model~\cite{c4model, brown-c4}. Контекстная диаграмма (рисунок~\ref{fig:c4-context}) представляет систему в окружении внешних акторов: инженера тестирования (управляет инфраструктурой через веб-интерфейс), потребителя (HTTP-клиент: нагрузочный инструмент или тестируемая система), Prometheus (сбор метрик) и целевого хоста Astra Linux~\cite{astra-linux} с Java-процессами~\cite{openjdk}.
...
Компонентная диаграмма (рисунок~\ref{fig:c4-component}) детализирует внутреннюю структуру REST (Representational State Transfer) API. Назначение шести компонентов приведено в таблице~\ref{tab:components}.
...
Crypto Service    & Шифрование/дешифрование паролей SSH алгоритмом AES-128 (Advanced Encryption Standard) (Fernet~\cite{cryptography-fernet}); ключ хранится в файле с правами 600 \\ \hline
Metrics Exporter  & маршрут \texttt{/metrics} в формате Prometheus exposition format~\cite{prometheus-docs, prometheus-exposition, turnbull-prometheus}; детали в разделе~2.6 \\ \hline
\end{xltabular}
}
...
\texttt{account\_uuid}  & UUID (Universally Unique Identifier)    & ID учётной записи SSH из реестра \texttt{accounts} \\ \hline
...
Все экранные формы разделяют единый шаблон-«обёртку»: постоянная боковая панель с тремя разделами (\textbf{Панель управления}, \textbf{Настройки}, \textbf{Журналы}) и верхняя строка с заголовком текущего раздела и индикатором подключения к Redis. Макет общего каркаса приложения представлен на рисунке~\ref{fig:mockup-shell}.
...
\subsection{Остановка процесса, проверка активности и пул SSH-соединений}
...
Прокси-роутер регистрирует два маршрута: \texttt{/\{mock\_name\}} и \texttt{/\{mock\_name\}/\{path:path\}}~-- оба делегируются единой функции \texttt{\_handle\_proxy}. Конвейер обслуживания включает следующие этапы:

\begin{compactlist}
\item выполнение \texttt{HMGET} пяти полей заглушки из Redis;
\item проверка статуса (\texttt{RUNNING}, иначе HTTP~503);
\item чтение глобальных параметров (\texttt{settings:global});
\item проверка Rate Limiter (при превышении~-- HTTP~429);
\item инкремент счётчика успешных запросов;
\item получение клиента из \texttt{ProxyClientRegistry};
\item чтение тела запроса (кроме GET/HEAD/OPTIONS);
\item вызов \texttt{proxy\_request} и возврат ответа;
\item сохранение метрик задержки в Redis.
\end{compactlist}

Прозрачность достигается следующим образом: метод, тело и параметры запроса передаются на целевой сервер без изменений; заголовки пересылаются полностью за исключением \texttt{Host} и \texttt{Content-Length}, которые формируются заново для нового соединения. Ошибки транспортного уровня унифицированы: \texttt{httpx.TimeoutException}~-- HTTP~504, любой \texttt{httpx.RequestError}~-- HTTP~502~\cite{fielding-dissertation}. Все промежуточные этапы фиксируются в словаре \texttt{timeline} с точностью 0.001~мс, что позволяет на уровне Prometheus-метрик разделить задержку Redis, Rate Limiter и отправкой на целевой хост.

% \subsection{Алгоритм Rate Limiting: Fixed Window Counter}

% Алгоритм реализован в классе \texttt{RateLimiter}: для каждой заглушки и каждого временного окна хранится счётчик по ключу \texttt{rate:\{filename\}:\{window\}}. Метка окна~-- целочисленное деление Unix-timestamp на размер окна. Проверка требует двух атомарных операций Redis: INCR и (только при создании нового счётчика) EXPIRE. Реализация приведена в приложении~\ref{cha:appendix1} (листинг~\ref{lst:rate-limiter-impl}).
...
Включение и отключение Rate Limiter производится в реальном времени через PATCH-запрос без остановки заглушки~-- операция обновляет единственное поле \texttt{rate\_limit\_enabled} в Hash-записи Redis; новое значение применяется с первого же следующего запроса. Порог \texttt{rate\_limit} изменяется аналогично и вступает в силу с начала следующего временного окна. Методы API управления приведены в таблице~\ref{tab:rate-limit-api}.

{\small
\begin{xltabular}{\textwidth}{|l|l|X|}
\caption{API-методы управления Rate Limiter} \label{tab:rate-limit-api} \\
\hline
...
\texttt{hostname}    & Раскрывающийся список  & Целевой хост из реестра; список формируется сервером при генерации \\ \hline
...
\textbf{Имя метрики} & \textbf{Тип} & \textbf{Описание} \\ \hline
\endhead
\texttt{mockcontrol\_\-mocks\_\-total}                                 & gauge   & Число зарегистрированных заглушек \\ \hline
\texttt{mockcontrol\_\-mocks\_\-running}                               & gauge   & Число заглушек в состоянии RUNNING \\ \hline
\texttt{mockcontrol\_\-mocks\_\-errors}                                & gauge   & Число заглушек в состоянии ERROR \\ \hline
\texttt{mockcontrol\_\-hosts\_\-available}                             & gauge   & Число хостов в состоянии AVAILABLE \\ \hline
\texttt{mockcontrol\_\-proxy\_\-requests\_\-total}           & counter & Успешно проксированных запросов (метка \texttt{mock}) \\ \hline
\texttt{mockcontrol\_\-rate\_\-limit\_\-rejected\_\-total} & counter & Отклонённых запросов по Rate Limiter (метка \texttt{mock}) \\ \hline
\texttt{mockcontrol\_\-proxy\_\-stage\_\-latency\_\-ms\_\-sum}   & counter & Накопленная сумма задержек по этапам, мс (метки \texttt{mock}, \texttt{stage}) \\ \hline
\texttt{mockcontrol\_\-proxy\_\-stage\_\-latency\_\-ms\_\-count} & counter & Число наблюдений задержки по этапам (метки \texttt{mock}, \texttt{stage}) \\ \hline
\texttt{mockcontrol\_\-proxy\_\-stage\_\-latency\_\-ms\_\-avg}   & gauge   & Среднее время этапа, мс (метки \texttt{mock}, \texttt{stage}) \\ \hline
\texttt{mockcontrol\_\-proxy\_\-stage\_\-latency\_\-ms\_\-last}  & gauge   & Последнее измеренное время этапа, мс (метки \texttt{mock}, \texttt{stage}) \\ \hline
\texttt{mockcontrol\_\-proxy\_\-outcome\_\-total}            & counter & Исходы проксирования по типу (метки \texttt{mock}, \texttt{outcome}) \\ \hline
\texttt{mockcontrol\_\-proxy\_\-upstream\_\-status\_\-total}   & counter & Ответы заглушки по HTTP-коду (метки \texttt{mock}, \texttt{status}) \\ \hline
\end{xltabular}
}
