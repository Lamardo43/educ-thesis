...
Metrics Exporter  & Эндпоинт \texttt{/metrics} в формате Prometheus exposition format~\cite{prometheus-docs, prometheus-exposition, turnbull-prometheus}; детали в разделе~2.6 \\ \hline
\end{xltabular}
}
...
Все экранные формы разделяют единый шаблон-«обёртку»: постоянная боковая панель с тремя разделами (\textbf{Панель управления}, \textbf{Настройки}, \textbf{Журналы}) и верхняя строка с заголовком текущего раздела и индикатором подключения к Redis. Макет общего каркаса приложения представлен в приложении Б (рисунок~\ref{fig:mockup-shell}).
