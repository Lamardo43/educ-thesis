...
Все экранные формы разделяют единый шаблон-«обёртку»: постоянная боковая панель с тремя разделами (\textbf{Dashboard}, \textbf{Настройки}, \textbf{Журналы}) и верхняя строка с заголовком текущего раздела и индикатором подключения к Redis. Макет общего каркаса приложения представлен в приложении Б (рисунок~\ref{fig:mockup-shell}).
...
Прокси-роутер регистрирует два catch-all маршрута: \texttt{/\{mock\_name\}} и \texttt{/\{mock\_name\}/\{path:path\}}~-- оба делегируются единой функции \texttt{\_handle\_proxy}. Конвейер обслуживания включает следующие этапы:
